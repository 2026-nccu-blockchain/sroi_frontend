...
import { ID_CARD_ACCEPT, ID_CARD_MAX_SIZE } from "@/modules/profile/constants";
...
// 先在前端擋掉後端一定會拒絕的檔案（格式、5MB）
const handleFileChange = (event: Event): void => {
  const input = event.target as HTMLInputElement;
  const selected = input.files?.[0] ?? null;

  let fileError = "";
  if (selected && !ID_CARD_ACCEPT.includes(selected.type)) fileError = "僅支援 JPG、PNG、WebP 圖片";
  else if (selected && selected.size > ID_CARD_MAX_SIZE) fileError = "圖片不可超過 5MB";
  ...
  <form class="form" novalidate @submit.prevent="handleSubmit">
    ...
    <div class="field">
      ...
      <label class="upload" :class="{ 'field__input--error': errors.file }">
        <img v-if="previewUrl" :src="previewUrl" alt="證件照片預覽" class="upload__preview" />
        <span v-else class="upload__placeholder">點擊選擇學生證或教職員證照片</span>
        <input type="file" :accept="ID_CARD_ACCEPT.join(',')" class="upload__input" @change="handleFileChange" />
      </label>
      <span v-if="errors.file" class="field__error">{{ errors.file }}</span>
      <span v-else class="field__hint">支援 JPG、PNG、WebP，檔案大小 5MB 以內</span>
    </div>
